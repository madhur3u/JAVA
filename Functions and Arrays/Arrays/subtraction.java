...
public class Main{
    public static void main(String[] args) throws Exception {
        ...
        int n2 = scn.nextInt();
        int[] b = new int[n2];
        for (int i = 0; i< n2; i ++)    b[i] = scn.nextInt();   // array 2 input
        ...
        while (k >= 0) {                                        // loop runs till largest number exists
          ...
          int d = b[j] - borrow;                                // a will be subtracted from b so giving value b to d already and 
                                                                // subtracting borrow it may have in previous iteration
        ...
        int index0 = 0;                                         // there might be trailing zeroes so we have to read from index from which we
        while (diff[index0] == 0)  index0++;                    // find 1st non zero number in diff array 
